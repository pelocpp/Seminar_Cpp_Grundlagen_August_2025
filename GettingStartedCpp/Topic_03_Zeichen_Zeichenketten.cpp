...
static void mainChar()
{
    ...
    int n;    // Symbiose via ASCII Tabelle

    ch = '*';
    std::cout << ch << std::endl;

    n = ch;  // Wertzuweisung: char nach int
...
static void mainZeichenkette_C()
{
    // Typ einer konstanten Zeichenkette ist 'const char*'
    // Das ist Bestandteil von C
    const char* s = "ABCDEF";  // Zeichenkette (viele Zeichen) // Datentyp ?????????

    // Wie kann man komfortabler mit Zeichenketten arbeiten?
    // Objekt-orientiert: std::string
}

 void mainZeichenkette()
{
